...
class Bridge {
  ...
  async start() {
    // 1. Register or reuse existing token
    if (this.config.peertube.runnerToken) {
      this.runnerClient.runnerToken = this.config.peertube.runnerToken
      this.log.info('Using existing runner token from config')
    } else {
      const reg = await this.runnerClient.register()
      this.log.info({ runnerId: reg.id, runnerName: this.config.peertube.runnerName },
        'Registered with PeerTube at %s', this.config.peertube.url)
    }
